...
const OUTPUT_PIXEL_DIR = path.join(ROOT_DIR, "output", "art", "pixel", "portraits");
const PUBLIC_PORTRAIT_DIR = path.join(ROOT_DIR, "apps", "web", "public", "assets", "precinct", "portraits");
const GENERATED_CONTENT_DIR = path.join(ROOT_DIR, "packages", "content", "src", "generated");
const GENERATED_TS_PATH = path.join(GENERATED_CONTENT_DIR, "portraitArt.ts");
const OUTPUT_MANIFEST_PATH = path.join(OUTPUT_PIXEL_DIR, "manifest.json");
const PUBLIC_MANIFEST_PATH = path.join(ROOT_DIR, "apps", "web", "public", "assets", "precinct", "portrait-art-manifest.json");
const IMAGEGEN_SOURCE_DIRS = [
  path.join(ROOT_DIR, "output", "art", "imagegen", "portraits-core")
];
const PORTRAIT_OUTPUT_SIZE = 512;

const PORTRAIT_EXPORTS = [
  ...
  { portraitId: "shadow_portrait" },
  {
    ...
    fallbackAdjustments: {
      brightness: 6,
      contrast: 1.04,
      channelMultipliers: [1.12, 1.03, 0.9]
    }
  },
  {
    ...
    fallbackAdjustments: {
      ...
      brightness: 8,
      contrast: 1.02,
      channelMultipliers: [0.97, 1, 1.12]
    }
  },
  {
    ...
    fallbackAdjustments: {
      ...
      brightness: -6,
      contrast: 1.06,
      channelMultipliers: [1.04, 0.95, 0.93]
    }
  }
];
...
function printHelp() {
  console.log(`Portrait runtime asset exporter

Usage:
  node scripts/art/export-portrait-runtime-assets.mjs

This script exports portrait renders into runtime PNGs, copies them into
apps/web/public/assets/precinct/portraits, and generates a code manifest under
packages/content/src/generated/portraitArt.ts.
`);
}
...
function decodePng(buffer) {
  ...
  if (signature !== "89504e470d0a1a0a") {
    throw new Error("Expected a PNG file when decoding runtime portrait art.");
  }
...
function applyChannelAdjustments(rgba, adjustments) {
  if (!adjustments) {
    return rgba;
  }
  ...
  if (
    brightness === 0
    && contrast === 1
    && channelMultipliers[0] === 1
    && channelMultipliers[1] === 1
    && channelMultipliers[2] === 1
  ) {
    return rgba;
  }

  const adjusted = Buffer.from(rgba);

  for (let offset = 0; offset < adjusted.length; offset += 4) {
    if (adjusted[offset + 3] === 0) {
      continue;
    }

    for (let channel = 0; channel < 3; channel += 1) {
      const multiplied = adjusted[offset + channel] * channelMultipliers[channel];
      const contrasted = ((multiplied - 128) * contrast) + 128 + brightness;
      adjusted[offset + channel] = Math.max(0, Math.min(255, Math.round(contrasted)));
    }
  }
...
function flipRgbaHorizontally(rgba, width, height) {
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      rgba.copy(flipped, targetOffset, sourceOffset, sourceOffset + 4);
    }
  }
...
function applyFallbackAdjustments(rgba, width, height, adjustments) {
  if (!adjustments) {
    return rgba;
  }
  ...
  if (adjustments.flipX) {
    adjusted = flipRgbaHorizontally(adjusted, width, height);
  }

  return applyChannelAdjustments(adjusted, adjustments);
}
...
async function resolveSourceRender(entry) {
  const imagegenSourceIndex = await getImagegenSourceIndex();
  const directImagegenPath = imagegenSourceIndex.get(entry.portraitId);
  if (directImagegenPath) {
    return {
      sourceKind: "imagegen",
      sourceRenderPath: directImagegenPath,
      sourcePortraitId: entry.portraitId,
      fallbackSourcePortraitId: null,
      fallbackAdjustments: null
    };
  }

  if (entry.fallbackSourcePortraitId) {
    const fallbackImagegenPath = imagegenSourceIndex.get(entry.fallbackSourcePortraitId);
    if (fallbackImagegenPath) {
      return {
        sourceKind: "imagegen",
        sourceRenderPath: fallbackImagegenPath,
        sourcePortraitId: entry.fallbackSourcePortraitId,
        fallbackSourcePortraitId: entry.fallbackSourcePortraitId,
        fallbackAdjustments: entry.fallbackAdjustments ?? null
      };
    }
  }

  const imagegenPath = imagegenSourceIndex.get(entry.portraitId);
  if (imagegenPath) {
    return {
      sourceKind: "imagegen",
      sourceRenderPath: imagegenPath,
      sourcePortraitId: entry.portraitId,
      fallbackSourcePortraitId: null,
      fallbackAdjustments: null
    };
  ...
  const manualPath = path.join(ROOT_DIR, "output", "art", "renders", "manual", `${entry.portraitId}.png`);
  if (await pathExists(manualPath)) {
    return {
      sourceKind: "manual",
      sourceRenderPath: manualPath,
      sourcePortraitId: entry.portraitId,
      fallbackSourcePortraitId: null,
      fallbackAdjustments: null
    };
  ...
  const meshyPath = path.join(ROOT_DIR, "output", "art", "renders", "meshy", `${entry.portraitId}.png`);
  if (await pathExists(meshyPath)) {
    return {
      sourceKind: "meshy",
      sourceRenderPath: meshyPath,
      sourcePortraitId: entry.portraitId,
      fallbackSourcePortraitId: null,
      fallbackAdjustments: null
    };
...
async function convertPortraitAsset(entry) {
  const { portraitId } = entry;
  const source = await resolveSourceRender(entry);
  if (!source) {
    return null;
  }
  ...
  const {
    ...
    sourceRenderPath,
    sourcePortraitId,
    fallbackSourcePortraitId,
    fallbackAdjustments
  } = source;
  const outputFilePath = path.join(OUTPUT_PIXEL_DIR, `${portraitId}.png`);
  const publicFilePath = path.join(PUBLIC_PORTRAIT_DIR, `${portraitId}.png`);

  await run("sips", [
    "-z",
    String(PORTRAIT_OUTPUT_SIZE),
    String(PORTRAIT_OUTPUT_SIZE),
    sourceRenderPath,
    "--out",
    outputFilePath
  ...
  rgba = applyFallbackAdjustments(rgba, decoded.width, decoded.height, fallbackAdjustments);
  ...
  return {
    portraitId,
    sourcePortraitId,
    isFallback: fallbackSourcePortraitId !== null,
    fallbackSourcePortraitId,
    publicPath: `/assets/precinct/portraits/${portraitId}.png`,
...
function generateTypeScriptManifest(entries) {
  const portraitIds = PORTRAIT_EXPORTS.map((entry) => entry.portraitId);
  const sortedEntries = [...entries].sort((left, right) => left.portraitId.localeCompare(right.portraitId));
  const serializedPortraitIds = JSON.stringify(portraitIds, null, 2);
  const serializedEntries = JSON.stringify(sortedEntries, null, 2);

  return `export const portraitArtCatalogIds = ${serializedPortraitIds} as const;
...
const portraitArtEntries: PortraitArtAsset[] = ${serializedEntries};
...
async function main() {
  ...
  await fs.mkdir(PUBLIC_PORTRAIT_DIR, { recursive: true });
  await fs.mkdir(GENERATED_CONTENT_DIR, { recursive: true });

  const convertedEntries = [];
  const missingPortraitIds = [];

  for (const entry of PORTRAIT_EXPORTS) {
    const convertedEntry = await convertPortraitAsset(entry);
    if (convertedEntry) {
      convertedEntries.push(convertedEntry);
    } else {
      missingPortraitIds.push(entry.portraitId);
    }
  }
  ...
  console.log(`Exported ${convertedEntries.length} portrait runtime assets.`);
  if (missingPortraitIds.length > 0) {
    console.warn(`Skipped ${missingPortraitIds.length} missing portrait source renders: ${missingPortraitIds.join(", ")}`);
  } else {
    const fallbackEntries = convertedEntries
      .filter((entry) => entry.isFallback)
      .map((entry) => `${entry.portraitId}<=${entry.fallbackSourcePortraitId}`);
    if (fallbackEntries.length > 0) {
      console.warn(`Fallback portrait aliases used: ${fallbackEntries.join(", ")}`);
    }
  }
}
