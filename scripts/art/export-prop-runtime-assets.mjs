...
const OUTPUT_PIXEL_DIR = path.join(ROOT_DIR, "output", "art", "pixel", "props");
const PUBLIC_PROP_DIR = path.join(ROOT_DIR, "apps", "web", "public", "assets", "precinct", "props");
const GENERATED_CONTENT_DIR = path.join(ROOT_DIR, "packages", "content", "src", "generated");
const GENERATED_TS_PATH = path.join(GENERATED_CONTENT_DIR, "propArt.ts");
const OUTPUT_MANIFEST_PATH = path.join(OUTPUT_PIXEL_DIR, "manifest.json");
const PUBLIC_MANIFEST_PATH = path.join(ROOT_DIR, "apps", "web", "public", "assets", "precinct", "prop-art-manifest.json");
...
function printHelp() {
  console.log(`Prop runtime asset exporter

Usage:
  node scripts/art/export-prop-runtime-assets.mjs

This script exports prop renders into runtime PNGs, copies them into
apps/web/public/assets/precinct/props, and generates a code manifest under
...
function decodePng(buffer) {
  ...
  if (signature !== "89504e470d0a1a0a") {
    throw new Error("Expected a PNG file when decoding runtime prop art.");
  }
...
async function convertPropAsset(entry) {
  const { assetId, decorRole, outputSize, worldWidth, worldHeight } = entry;
  ...
  const publicFilePath = path.join(PUBLIC_PROP_DIR, `${assetId}.png`);
  ...
  return {
    assetId,
    decorRole,
    publicPath: `/assets/precinct/props/${assetId}.png`,
...
function generateTypeScriptManifest(entries) {
  ...
  return `export type PropArtAsset = {
  assetId: string;
  decorRole: string;
  publicPath: string;
  outputPath: string;
...
const propArtEntries: PropArtAsset[] = ${serializedEntries};
...
async function main() {
  ...
  await fs.mkdir(PUBLIC_PROP_DIR, { recursive: true });
  await fs.mkdir(GENERATED_CONTENT_DIR, { recursive: true });

  const convertedEntries = [];
  for (const entry of PROP_EXPORTS) {
    convertedEntries.push(await convertPropAsset(entry));
  }
  ...
  console.log(`Exported ${convertedEntries.length} prop runtime assets.`);
}
